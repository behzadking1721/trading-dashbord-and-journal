...
import { Save, Plus, Trash2, Edit, CheckCircle } from 'lucide-react';
import type { TradingSetup, TradingChecklistItem } from '../types';
...
const SettingsPage: React.FC = () => {
    ...
    const handleSaveRisk = () => {
        try {
            ...
            // Dispatch event to notify other components like WalletOverview
    ...
    const saveSetups = (updatedSetups: TradingSetup[]) => {
        try {
            ...
            setSetups(updatedSetups);
             // Dispatch event to notify checklist widget
    ...
    const handleSaveSetup = (setup: TradingSetup) => {
        ...
        const existingIndex = updatedSetups.findIndex(s => s.id === setup.id);
        
        if (existingIndex > -1) {
            updatedSetups[existingIndex] = setup;
        } else {
    ...
    const handleDeleteSetup = (id: string) => {
        if (window.confirm('آیا از حذف این ستاپ مطمئن هستید؟')) {
            saveSetups(setups.filter(s => s.id !== id));
        }
    };

    const handleSetActive = (id: string) => {
        saveSetups(setups.map(s => ({ ...s, isActive: s.id === id })));
    };
    ...
    return (
        <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-6 p-6 rounded-lg shadow-md bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700">
                    <div>
...
const SetupFormModal: React.FC<{setup: TradingSetup, onSave: (setup: TradingSetup) => void, onClose: () => void}> = ({ setup, onSave, onClose }) => {
    ...
    const handleAddItem = () => {
        if (newItemText.trim()) {
            const newItem: TradingChecklistItem = { id: Date.now().toString(), text: newItemText };
            setCurrentSetup(prev => ({ ...prev, checklist: [...prev.checklist, newItem]}));
            setNewItemText('');
        }
    };
    
    const handleDeleteItem = (id: string) => {
        setCurrentSetup(prev => ({ ...prev, checklist: prev.checklist.filter(item => item.id !== id)}));
    };
    ...
    return (
         <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl p-6 relative">
                 ...
                 <h2 className="text-xl font-bold mb-4">ویرایش ستاپ معاملاتی</h2>
